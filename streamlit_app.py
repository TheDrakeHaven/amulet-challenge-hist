...
with tab4:
    ...
    st.markdown(
        ...
        "derived by scaling the era median card counts proportionally to the target deck size. "
    ...
    # Use era median as the basis for the predicted decklist
    md_centroid = era_rows_t4[md_cols].median()
    sb_centroid = era_rows_t4[sb_cols_t4].median()
...
with tab10:
    ...
    if species_nmds10 is not None:
        ...
        st.info("NMDS site scores not found in data. Check that merged_amulet.csv contains NMDS1 and NMDS2 columns.")import streamlit as st
import pandas as pd
import numpy as np
import plotly.express as px
import plotly.graph_objects as go
