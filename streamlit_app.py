...
with tab8:
    ...
    if ord_nmds is not None:
        ...
        fig_nmds.update_traces(marker=dict(size=6, line=dict(width=0)))
        fig_nmds.update_xaxes(title_text="NMDS1 (no units)", showgrid=True, gridcolor="#eee", zeroline=False)
        fig_nmds.update_yaxes(title_text="NMDS2 (no units)", showgrid=True, gridcolor="#eee", zeroline=False)
        ...
        if show_centroids_nmds and color_by_nmds in centroids_nmds:
            ...
            fig_nmds.add_trace(go.Scatter(
                ...
                name="Centroids",
                showlegend=True,
                hovertext=cents["label"],
                hoverinfo="text",
            ))
            # Annotation arrows so labels don't pile up
            for _, crow in cents.iterrows():
                fig_nmds.add_annotation(
...
with tab10:
    ...
    st.markdown(
        ...
        "Cards that appear together in similar decklists cluster together. "
    ...
    if _nmds_species is not None:
        ...
        if color_mode10 == "Card type":
            ...
            filtered10["_color"] = filtered10["card_type"]
        else:
        ...
        fig10.update_traces(mode="markers+text", textposition="top center", marker=dict(size=8))
        fig10.update_layout(
            title="Card WA Scores (NMDS)",
            xaxis_title="NMDS1 (no units)",
            yaxis_title="NMDS2 (no units)",
            template="simple_white",
            height=1000,
